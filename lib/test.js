...
var _1 = require(".");
var ensure = new _1.RandomEnsure([0, 1, 2, 3], {
    ensure: 0.25
});
var results = {};
console.time("100k shuffle() test ran in");
for (var i = 0; i < 100000; i++) {
    var old = ensure.list.slice(ensure.list.length - ensure.options.ensure);
    ensure.shuffle();
    for (var ii = 0; ii < old.length; ii++) {
        if (ensure.list.indexOf(old[ii]) < ensure.options.ensure) {
            console.log(old, ensure.list);
            throw new Error("Invalid shuffle() ensure");
        }
    }
    var key = ensure.list.join("");
    if (results[key]) {
        results[key]++;
    }
    else {
